...
<section class=" tw-pt-7.5">
    ...
    <div class=" tw-container">
        <div class=" tw-flex tw-gap-1 tw-mb-7.5">
            <span @click="$router.push({name:'home'})" class=" tw-cursor-pointer tw-underline">
                Main
            </span>
            <span>
                →
            </span>
            <span class=" tw-text-dark-blue-2">
                Tokenomics
            </span>
        </div>
        <div class=" tw-flex tw-justify-between tw-mb-10">
            ...
            <div class=" tw-flex tw-flex-col" v-if="$q.screen.xl">
                ...
                <span class=" tw-text-purple-light">
                    1 000 000 000 ALG
                </span>
            </div>
        </div>
        <div class=" tw-flex tw-items-center tw-justify-center xl:tw-justify-between ">
            <ul class=" tw-flex tw-flex-col tw-gap-2.5" v-if="$q.screen.xl">
                <li v-for="(item, index) in listLeft" :key="index" class="tokenomic tw-flex tw-items-center tw-gap-5">
                    <HexahedronIcon :fill="item.color" :text="item.text" textAfter="%"/>
                    <span>
                        {{item.name}}
                    </span>
                </li>
                ...
                <RoundDiagram class="inside__round" @clickCircle="clickCircle" :values="allValue"
                    ...
                    <div class="round__card" >
                        <transition name="bounce">
                            <div v-show="show" v-if="cardData" ref="card" class="card card__border-line" >
                                <div class=" tw-flex">
                                    <div>
                                        <h4 class=" tw-mb-2.5">{{cardData?.name}}</h4>
                                        <p class=" tw-text-purple-light">{{cardData?.description}}</p>
                                        <p v-if="cardData?.price" class=" tw-text-white tw-text-xxs">{{cardData.price}}</p>
                                    </div>
                                    <div class="round__card-right tw-relative">
                                        <HexahedronIcon class="round__card-icon" :fill="cardData.color" :text="cardData.text" textAfter="%"/>
                                        <img v-if="!$q.screen.xl" @click="unHovered" :src="require('assets/icons/close.svg')" alt="icon-close" class="round__card-close">
                                    </div>
                                </div>
                            </div>
                        </transition>
                    </div>
                    <div v-show="show" class="round__wrapper">
                    </div>

            </div>
            <ul class=" tw-flex tw-flex-col tw-gap-2.5" v-if="$q.screen.xl">
                <li v-for="(item, index) in listRight" :key="index" class="tokenomic tw-flex tw-items-center tw-gap-5">
                    <HexahedronIcon :fill="item.color" :text="item.text" textAfter="%"/>
                    <span>
                        {{item.name}}
                    </span>
...
export default {
    ...
    setup(){
        ...
        const clickAnotherWindow = (e)=>{
            if(show.value && !e.target.closest('circle') && !e.composedPath().includes(card.value)){
                show.value = false
