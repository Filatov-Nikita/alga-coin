...
export default function (){
    const dataGenerate = (times, values)=>times.map((item,index)=>{return [item,values[index]]})
    const formatingDate = computed(()=>timeValues.map(item=> item*1000))
    const lastTwoMontPresent = (array) => ((array[array.length-1]/array[0])*100-100).toFixed(2)
    console.log(formatingDate.value);
    console.log(lastTwoMontPresent(btnMain) );
    return {
        ...
        stableIndexData: computed(()=>dataGenerate(formatingDate.value, stableIndex)),
