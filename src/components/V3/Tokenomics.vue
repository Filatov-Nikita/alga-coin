<template>
  <div class="xl:tw-flex xl:tw-justify-between">
    <div>
      <div
        class="tw-border-b tw-border-solid tw-border-text-gray tw-pb-5 tw-pl-4 tw-flex tw-gap-2 tw-mb-6"
      >
        <div>{{ t("total") }}:</div>
        <p>1 000 000 000 ALG</p>
      </div>
      <ul
        class="tw-flex tw-flex-col xl:tw-grid xl:tw-grid-cols-3 tw-gap-2.5 xl:tw-gap-y-5"
        v-if="$q.screen.xl"
      >
        <li
          ...
          @click="clickTocken"
        >
          <SquareIcon :fill="item.color" :text="item.text" textAfter="%" />
          <span>
            {{ item.name }}
          </span>
        </li>
...
import SquareIcon from "src/components/V3/SquareIcon.vue";
